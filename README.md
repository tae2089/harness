...
cp -r skills/gemini-harness ~/.gemini/skills/gemini-harness
```

> **심볼릭 링크 권장 (업데이트 자동 반영):**
> ```bash
> ln -s "$(pwd)/skills/gemini-harness" ~/.gemini/skills/gemini-harness
> ```

### 2. 프로젝트 수준 설치 (해당 프로젝트에서만 사용)

```bash
cd {프로젝트 루트}
mkdir -p .gemini/skills
cp -r {harness_repo}/skills/gemini-harness .gemini/skills/gemini-harness
```

### 3. 설치 확인

```bash
ls ~/.gemini/skills/gemini-harness/SKILL.md
# → 파일 존재 시 설치 완료
```

Gemini CLI 세션에서 `"하네스 구성해줘"` 발화 → `gemini-harness` 스킬 자동 트리거 확인.

> **업데이트:** 심볼릭 링크 사용 시 `git pull`만으로 자동 반영. 복사 설치는 `cp -r`로 재복사.
