...
- **7대 패턴:** Pipeline, Fan-out/Fan-in, Expert Pool, Producer-Reviewer, Supervisor, Hierarchical, Handoff — 단일 또는 다단계 Stage-Phase 워크플로우로 조합
...
│           └── references/     # 참조 문서
├── _workspace/
│   ├── workflow.md             # Stage-Phase 구조 선언 (모든 하네스 필수)
│   ├── findings.md             # 데이터 브로커 (에이전트 간 통찰 공유)
│   ├── tasks.md                # 태스크 보드
│   └── checkpoint.json         # 재개 지점 저장
...
- `references/stage-phase-guide.md` — Stage-Phase 워크플로우 가이드 (workflow.md 명세, 전환 프로토콜, 예시 3종)
- `references/evolution-protocol.md` — 하네스 진화, 피드백 반영, 운영/유지보수
- `references/expansion-matrix.md` — 기존 확장 시 Phase 선택 매트릭스
