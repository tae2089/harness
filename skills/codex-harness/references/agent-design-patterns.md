...
## sandbox_mode 권한 제어 가이드

Codex CLI는 `tools:` 배열 대신 **`sandbox_mode`** 단일 필드로 에이전트 권한을 제어한다. 역할에 맞는 최소 권한 원칙을 따른다.

### sandbox_mode별 가용 작업

| sandbox_mode          | 가용 작업                                                              | 대표 에이전트              |
| --------------------- | ---------------------------------------------------------------------- | -------------------------- |
| `read-only`           | shell `cat`·`find`·`grep`·`ls`, web fetch — 파일 쓰기 불가            | Analyst, Architect         |
| `workspace-write`     | + `apply_patch`, shell `tee`·write, subagent spawn, test 실행         | Coder, Reviewer, State Manager |
| `danger-full-access`  | + 외부 프로세스 (kubectl, terraform, deploy 등)                        | Operator, Deployer         |
