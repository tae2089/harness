...
10. [에이전트 정의 구조](#에이전트-정의-구조-codex-cli-공식-포맷)
...
Codex CLI에서는 메인 에이전트가 **오케스트레이터**가 되어 서브에이전트들을 조율한다. 서브에이전트 간 직접 통신은 불가능하지만, 메인 에이전트의 **중개(brokering)**와 공유 파일을 통해 고도의 협업을 구현할 수 있다.
...
> **Claude Code와의 차이점:** Claude Code에는 `TeamCreate`/`SendMessage`/`TaskCreate` 기반의 "에이전트 팀" 모드가 존재하여 팀원끼리 직접 대화·브로드캐스트할 수 있지만, **Codex CLI에는 이런 팀 API가 없다**. 팀원 간 통신이 필요하면 메인 에이전트가 shell `cat`로 산출물을 읽고 다음 에이전트 프롬프트에 주입하는 **파일 중개** 방식으로 대체한다. 또한 서브에이전트 호출 시 **`spawn_subagent` 도구**를 사용하며, 병렬 실행이 필요한 경우 **`wait_for_previous: false`** 파라미터를 지정하여 단일 턴 내 동시 실행을 구현해야 한다. (셸 명령 자체의 백그라운드 실행은 `run_shell_command`로 별개 지원.)
...
> **핵심 원칙:** Codex CLI는 "메인 에이전트가 유일한 브로커"라는 전제가 모든 패턴의 기반이다. 패턴 선택 시 "메인이 어떤 시점에 어떤 정보를 중개해야 하는가?"를 먼저 설계한다.
...
- **사용 예시 (오케스트레이터의 호출 방식):**
  > **메인:** `[Tool: spawn_subagent(agent_name="analyst", wait_for_previous=true)]` `@{analyst}`님, 요구사항을 분석하여 `_workspace/{plan_name}/01_reqs.md`에 저장하세요. 필요시 `스킬 로드('context7-docs')`를 활용해 최신 명세를 확인하세요.
  > (완료 후)
  > **메인:** `[Tool: spawn_subagent(agent_name="coder", wait_for_previous=true)]` `@{coder}`님, `_workspace/{plan_name}/01_reqs.md`를 읽고 `findings.md`의 [핵심 제약]을 준수하여 구현하세요. 결과는 `_workspace/{plan_name}/02_code.md`에 저장하세요.
...
- **사용 예시 (오케스트레이터의 호출 방식):**
  > **메인:** (단일 턴 내 배치 호출)
  >
  > 1. `[Tool: spawn_subagent(agent_name="researcher-market", wait_for_previous=false)]` `@{researcher-market}`님, 시장 트렌드를 조사해 `_workspace/{plan_name}/mkt.md`에 기록하세요.
  > 2. `[Tool: spawn_subagent(agent_name="researcher-tech", wait_for_previous=false)]` `@{researcher-tech}`님, 기술 타당성을 조사해 `_workspace/{plan_name}/tech.md`에 기록하세요.
  >    (모두 완료 후)
  >    **메인:** `[Tool: spawn_subagent(agent_name="strategist", wait_for_previous=true)]` `@{strategist}`님, `_workspace/{plan_name}/mkt.md`와 `_workspace/{plan_name}/tech.md`를 모두 읽고 `findings.md`에 상충하는 데이터가 있다면 해결하여 최종 전략을 수립하세요.
...
- **사용 예시 (오케스트레이터의 호출 방식):**
  ...
  > **메인:** `[Tool: spawn_subagent(agent_name="perf-expert", wait_for_previous=true)]` `@{perf-expert}`님, 현재 코드를 분석하여 병목 지점을 찾으세요. `@{security-expert}`님은 이번 작업에서 제외됩니다.
- **적합:** 다국어 지원(언어별 네이티브 에이전트), 코드 리뷰(보안/성능/아키텍처 전문가 선택), 기술 스택별 특화 리뷰.
- **주의:** 라우터(메인)의 분류 정확도가 핵심. 분류가 모호하면 사용자 확인 요청으로 명확화하거나, 두 전문가를 동시 호출해 교차 검증.
...
- **사용 예시 (오케스트레이터의 호출 방식):**
  > **메인:** `[Tool: spawn_subagent(agent_name="writer", wait_for_previous=true)]` `@{writer}`님, 초안을 작성하세요.
  > **메인:** `[Tool: spawn_subagent(agent_name="editor", wait_for_previous=true)]` `@{editor}`님, 초안을 리뷰하고 `스킬 로드('qa-standard')`에 따라 수정 제안을 `_workspace/{plan_name}/edit_notes.md`에 남기세요.
  > (리뷰 결과 'Fail'인 경우)
  > **메인:** `[Tool: spawn_subagent(agent_name="writer", wait_for_previous=true)]` `@{writer}`님, `_workspace/{plan_name}/edit_notes.md`의 피드백을 반영해 초안을 수정하세요. (최대 2회 재시도(총 3회))
- **적합:** 웹툰(artist→reviewer→재생성), 고품질 코드 작성, 기술 문서 교정.
- **주의:** **무한 루프 방지를 위해 최대 2회 재시도(총 3회) 설정 필수.** 3회 후에도 실패 시 메인이 사용자 확인 요청으로 사람에게 결정권을 넘긴다.
...
- **사용 예시 (오케스트레이터의 호출 방식):**
  ...
  >
  > 1. `[Tool: spawn_subagent(agent_name="worker-1", wait_for_previous=false)]` `@{worker-1}`님, 작업 ID 1~10번을 수행하세요.
  > 2. `[Tool: spawn_subagent(agent_name="worker-2", wait_for_previous=false)]` `@{worker-2}`님, 작업 ID 11~20번을 수행하세요.
  >    (완료 보고가 오면)
...
- **사용 예시 (오케스트레이터의 호출 방식):**
  > **메인:** `[Tool: spawn_subagent(agent_name="frontend-lead", wait_for_previous=true)]` `@{frontend-lead}`님, UI 시스템 구축을 총괄하세요.
  > **@{frontend-lead}:** (프롬프트 내에서) `[Tool: spawn_subagent(agent_name="component-coder", wait_for_previous=false)]` `@{component-coder}`님은 버튼 컴포넌트를, `[Tool: spawn_subagent(agent_name="style-coder", wait_for_previous=false)]` `@{style-coder}`님은 테마 시스템을 구현하게 지시하겠습니다.
...
- **사용 예시 (오케스트레이터의 호출 방식):**
  > **메인:** `[Tool: spawn_subagent(agent_name="log-analyzer", wait_for_previous=true)]` `@{log-analyzer}`님, 에러 로그를 분석하고 근본 원인을 파악하세요. 전문 처리가 필요하면 산출물 끝에 `[NEXT_AGENT: @{agent-name}] 사유: {구체적 이유}`를 반드시 남기세요.
  > (log-analyzer 완료, 산출물 끝: `[NEXT_AGENT: @security-patcher] 사유: JWT 검증 누락으로 인한 인증 취약점 발견`)
  > **메인:** (키워드 파싱 후) `[Tool: spawn_subagent(agent_name="security-patcher", wait_for_previous=true)]` `@{security-patcher}`님, `_workspace/{plan_name}/log_analysis.md`를 읽고 식별된 인증 취약점을 패치하세요.
- **적합:** 탐색적 디버깅(로그 분석 → 재현 시도 → 수정 전문가 핸드오프), 복잡한 지원 프로세스.
- **주의:** 순환 핸드오프(A→B→A) 무한 루프 방지 필수 — 오케스트레이터는 호출 이력을 추적하고 동일 에이전트가 이미 실행됐으면 재소환하지 않는다. 핸드오프 연쇄가 3단계 이상 발생하거나 추천 대상 에이전트가 없으면 사용자 확인 요청으로 결정권 이전. 감지 pseudocode: `references/orchestrator-procedures.md` — "handle_handoff" 참조.
...
### 다단계 워크플로우 조율 포인트 (Codex CLI)

| 시나리오               | 브로커링 전략                                                                        | 주의                                                                                  |
| ---------------------- | ------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------- |
| **리서치 + 분석**      | 조사자들 산출물을 `findings.md`에 실시간 병합, 상충 발견은 [데이터 충돌] 섹션에 기록 | 통합 에이전트가 충돌을 해결하지 못하면 사용자 확인 요청 호출                                |
...
> **공통 규칙(모든 에이전트 필수 포함):**
>
> - 사용자 확인 요청: 모호한 지시·누락된 파라미터·데이터 충돌 시 추측하지 않고 사용자에게 확인 질의.
> - 스킬 로드: `.codex/skills/` 아래 절차 스킬(방법론·체크리스트·프로토콜)을 런타임에 로드·실행하기 위해 필수. 스킬을 호출하지 않는 에이전트는 원칙적으로 존재하지 않는다.

### 에이전트 유형별 표준 도구 세트

| 에이전트 유형 | 핵심 역할                 | 권장 도구 세트                                                                                                                                          | 관련 MCP/Built-in         |
| :------------ | :------------------------ | :------------------------------------------------------------------------------------------------------------------------------------------------------ | :------------------------ |
| **Analyst**   | 정보 수집, 분석, 리서치   | 사용자 확인 요청, 스킬 로드, shell `cat`, `read_many_files`, `list_directory`, `grep_search`, `google_web_search`, `web_fetch`                         | Search, Browser, Context7 |
| **Architect** | 구조 설계, 계획 수립      | 사용자 확인 요청, 스킬 로드, shell `cat`, `list_directory`, shell `find`, `grep_search`, `enter_plan_mode`, `exit_plan_mode`                                 | Planning, CCG             |
| **Coder**     | 코드 작성, 수정, 리팩토링 | 사용자 확인 요청, 스킬 로드, shell `cat`, `apply_patch`, `replace`, `run_shell_command` (format/lint)                                                   | Filesystem, Lang-specific |
| **Reviewer**  | 품질 검수, 테스트, QA     | 사용자 확인 요청, 스킬 로드, shell `cat`, `read_many_files`, `grep_search`, `run_shell_command` (test execution, 장기 프로세스는 백그라운드 옵션 활용) | Testing, Linter           |
| **Operator**  | 인프라 관리, 배포, CICD   | 사용자 확인 요청, 스킬 로드, `run_shell_command`, `k8s_*`, `terraform_*`                                                                               | K8s, Terraform, CLI       |

### 특수 목적 도구

Codex CLI에서 확인된 특수 도구. 일반 워커 에이전트에는 부여하지 않고 필요한 역할에만 선택적으로 허용한다.

| 도구                | 용도                                        | 권장 에이전트                                                                  |
| ------------------- | ------------------------------------------- | ------------------------------------------------------------------------------ |
| `spawn_subagent`      | 서브에이전트 호출                           | 오케스트레이터, Supervisor, Hierarchical 팀장(Mid-level) 전용 — 일반 워커 금지 |
| `save_memory`       | 세션 간 핵심 정보 영속화                    | 오케스트레이터 (체크포인트 보완용)                                             |
| `write_todos`       | 내장 할 일 목록 관리 (`tasks.md` 경량 대안) | 단일 에이전트 작업 시 오케스트레이터                                           |
| `get_internal_docs` | Codex CLI 내부 문서 조회                   | Analyst, Architect                                                             |
...
- **문서화 전문가:** `web_fetch`와 shell `cat` 위주로 할당하여 지식 베이스 구축.
...
|--------------|---------|-----------------|
| 오케스트레이터, Architect | **thinking 티어** (`gpt-5.2-thinking`) | `references/schemas/models.md` (SoT) |
| Coder, Analyst, Reviewer, Operator | **codex 티어** (`gpt-5.3-codex`) | `references/schemas/models.md` (SoT) |
| State Manager | **mini 티어** (`gpt-5.3-chat-latest`) | `references/schemas/models.md` (SoT) |
...
| 스타일                         | 호출 방식      | 특징                                                         | 적합한 경우                                             |
| :----------------------------- | :------------- | :----------------------------------------------------------- | :------------------------------------------------------ |
| **완전 위임 (Delegation)**     | `spawn_subagent` | 작업의 시작부터 끝(파일 생성)까지 서브에이전트가 완결.       | 코딩, 리서치, 테스트 실행 등 독립적 실무.               |
| **전문가 자문 (Consultative)** | `spawn_subagent` | 산출물 대신 **"분석 의견"**이나 **"체크리스트"**만 반환받음. | 아키텍처 리뷰, 보안 점검, 구현 전략 수립.               |
...
- **도구 격리의 힘:** 자문 스타일의 에이전트는 `apply_patch` 권한을 빼고 shell `cat`만 주어, 실수로 코드를 건드리지 않고 오직 "지식"만 제공하도록 설계할 수 있다.
...
**Codex CLI 추가 고려사항:** 분리할 때마다 메인 에이전트의 중개 부담이 늘어난다(각 에이전트 호출 전후로 `findings.md` 갱신 필요). 분리 이득이 중개 오버헤드를 상회하는지 점검할 것. 의심스러우면 처음엔 통합된 에이전트로 시작하고, 재사용 요구가 생길 때 분리한다.

---

## 에이전트 정의 구조 (Codex CLI 공식 포맷)

> **에이전트 파일 생성 시:** 인라인 예시가 아닌 **`_workspace/_schemas/agent-worker.template.toml`** (워커) 또는 **`agent-orchestrator.template.md`** (오케스트레이터)를 shell `cat`로 읽어 변수 치환 후 생성. 모델 ID는 반드시 **`_workspace/_schemas/models.md`** 확인 후 사용 — 임의 추측 금지.
...
> **주의:** Claude Code 에이전트 정의의 "팀 통신 프로토콜(SendMessage)" 섹션은 Codex CLI에 없다. 대신 "입력/출력 프로토콜"에서 파일 경로·완료 신호·브리핑 방식을 명시하는 것이 같은 역할을 한다.
...
| **위치**      | `.codex/skills/{name}/SKILL.md`                                | `.codex/agents/{name}.md`                 |
| **트리거**    | 사용자 요청 키워드 매칭 또는 에이전트가 스킬 로드로 호출 | 오케스트레이터가 `@{name}`으로 명시적 호출 |
...
| 방식                    | 구현                                                                                      | 적합한 경우                                               |
| ----------------------- | ----------------------------------------------------------------------------------------- | --------------------------------------------------------- |
| **스킬 로드 호출** | 에이전트 프롬프트에 "해당 절차는 `스킬 로드('{skill-name}')`을 호출해 실행하라" 명시 | 스킬이 독립 워크플로우이고 사용자가 직접 호출 가능한 경우 |
| **프롬프트 내 인라인**  | 에이전트 정의 본문에 스킬 내용을 직접 포함                                                | 스킬이 짧고(50줄 이하) 이 에이전트 **전용**인 경우        |
| **레퍼런스 로드**       | shell `cat`로 `.codex/skills/{skill}/references/*.md`를 필요 시 로드                     | 스킬 내용이 크고 **조건부로만** 필요한 경우               |

**권장:**

- 재사용성이 높으면 → 스킬 로드
- 전용이면 → 인라인
- 대용량 + 조건부면 → shell `cat` 레퍼런스 로드

**원칙:** 모든 에이전트 정의는 `tools` 배열에 스킬 로드를 포함한다. 스킬을 전혀 호출하지 않는 순수 페르소나형 에이전트는 원칙적으로 존재하지 않는다(행동 원칙·프로토콜·체크리스트는 거의 항상 스킬로 분리해야 재사용성·유지보수성이 높아지기 때문).
