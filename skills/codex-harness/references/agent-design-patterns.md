...
> **Claude Code와의 차이점:** Claude Code에는 `TeamCreate`/`SendMessage`/`TaskCreate` 기반의 "에이전트 팀" 모드가 존재하여 팀원끼리 직접 대화·브로드캐스트할 수 있지만, **Codex CLI에는 이런 팀 API가 없다**. 팀원 간 통신이 필요하면 메인 에이전트가 shell `cat`로 산출물을 읽고 다음 에이전트 프롬프트에 주입하는 **파일 중개** 방식으로 대체한다. 서브에이전트는 오케스트레이터가 `@agent-name` 지시로 호출하며, 병렬 실행이 필요한 경우 단일 턴 내 연속 호출로 구현한다. (셸 명령 자체의 백그라운드 실행은 `&`로 별개 지원.)
...
여러 에이전트가 병렬로 실행될 때, 공통 파일인 `tasks.md`에 동시에 쓰기를 시도하면 데이터가 유실될 수 있다. 이를 방지하기 위해 다음 **원자적 작업 관리(Atomic Task Management)** 원칙을 따른다.
...
- **워크플로우:**
  ...
  3. 메인이 `shell ls _workspace/tasks/`로 스캔하여 모든 개별 파일을 읽음.
...
- **사용 예시 (오케스트레이터의 호출 방식):**
  > **메인:** `` `@{analyst}`님, 요구사항을 분석하여 `_workspace/{plan_name}/01_reqs.md`에 저장하세요. 필요시 최신 명세를 확인하세요.
(완료 후)
**메인:** `` `@{coder}`님, `_workspace/{plan_name}/01_reqs.md`를 읽고 `findings.md`의 [핵심 제약]을 준수하여 구현하세요. 결과는 `_workspace/{plan_name}/02_code.md`에 저장하세요.
...
- **사용 예시 (오케스트레이터의 호출 방식):**
  > **메인:** (단일 턴 내 배치 호출)
  >
  > 1. `` `@{researcher-market}`님, 시장 트렌드를 조사해 `\_workspace/{plan_name}/mkt.md`에 기록하세요.
  > 2. `` `@{researcher-tech}`님, 기술 타당성을 조사해 `_workspace/{plan_name}/tech.md`에 기록하세요.
(모두 완료 후)
**메인:** `` `@{strategist}`님, `_workspace/{plan_name}/mkt.md`와 `_workspace/{plan_name}/tech.md`를 모두 읽고 `findings.md`에 상충하는 데이터가 있다면 해결하여 최종 전략을 수립하세요.
...
- **호출 방식:** 항상 순차 호출. 복수 전문가를 동시 호출하지 않는다.
- **사용 예시 (오케스트레이터의 호출 방식):**
  > **메인:** (사용자 요청 "React 성능 개선")
  > **메인:** `` `@{perf-expert}`님, 현재 코드를 분석하여 병목 지점을 찾으세요. `@{security-expert}`님은 이번 작업에서 제외됩니다.
...
- **사용 예시 (오케스트레이터의 호출 방식):**
  > **메인:** `` `@{writer}`님, 초안을 작성하세요.
**메인:** `` `@{editor}`님, 초안을 리뷰하고 qa-standard 기준에 따라 수정 제안을 `_workspace/{plan_name}/edit_notes.md`에 남기세요.
  > (리뷰 결과 'Fail'인 경우)
  > **메인:** `` `@{writer}`님, `\_workspace/{plan_name}/edit_notes.md`의 피드백을 반영해 초안을 수정하세요. (최대 2회 재시도(총 3회))
...
- **사용 예시 (오케스트레이터의 호출 방식):**
  ...
  >
  > 1. `` `@{worker-1}`님, 작업 ID 1~10번을 수행하세요.
  > 2. `` `@{worker-2}`님, 작업 ID 11~20번을 수행하세요.
(완료 보고가 오면)
...
- **사용 예시 (오케스트레이터의 호출 방식):**
  > **메인:** `` `@{frontend-lead}`님, UI 시스템 구축을 총괄하세요.
**@{frontend-lead}:** (프롬프트 내에서) `` `@{component-coder}`님은 버튼 컴포넌트를, `` `@{style-coder}`님은 테마 시스템을 구현하게 지시하겠습니다.
...
- **사용 예시 (오케스트레이터의 호출 방식):**
  > **메인:** `` `@{log-analyzer}`님, 에러 로그를 분석하고 근본 원인을 파악하세요. 전문 처리가 필요하면 산출물 끝에 `[NEXT_AGENT: @{agent-name}] 사유: {구체적 이유}`를 반드시 남기세요.
(log-analyzer 완료, 산출물 끝: `[NEXT_AGENT: @security-patcher] 사유: JWT 검증 누락으로 인한 인증 취약점 발견`)
**메인:** (키워드 파싱 후) `` `@{security-patcher}`님, `_workspace/{plan_name}/log_analysis.md`를 읽고 식별된 인증 취약점을 패치하세요.
...
| **생성 + 검증**        | 생성 → 검증 리포트(`qa_report.md`) → 재생성 시 리포트를 프롬프트에 주입              | 최대 2회 재시도(총 3회), 초과 시 사용자 확인 요청                                     |
...
| 도구                | 용도                                        | 권장 에이전트                                                                  |
| ------------------- | ------------------------------------------- | ------------------------------------------------------------------------------ |
| `@agent-name` 호출  | 서브에이전트 호출                           | 오케스트레이터, Supervisor, Hierarchical 팀장(Mid-level) 전용 — 일반 워커 금지 |
...
- **문서화 전문가:** shell `curl`·`cat` 위주로 할당하여 지식 베이스 구축.
...
| 스타일                         | 호출 방식         | 특징                                                         | 적합한 경우                                             |
| :----------------------------- | :---------------- | :----------------------------------------------------------- | :------------------------------------------------------ |
| **완전 위임 (Delegation)**     | 서브에이전트 호출 | 작업의 시작부터 끝(파일 생성)까지 서브에이전트가 완결.       | 코딩, 리서치, 테스트 실행 등 독립적 실무.               |
| **전문가 자문 (Consultative)** | 서브에이전트 호출 | 산출물 대신 **"분석 의견"**이나 **"체크리스트"**만 반환받음. | 아키텍처 리뷰, 보안 점검, 구현 전략 수립.               |
...
| **위치**      | `.codex/skills/{name}/SKILL.md`                          | `.codex/agents/{name}.toml`                |
...
| 방식                   | 구현                                                                                 | 적합한 경우                                               |
| ---------------------- | ------------------------------------------------------------------------------------ | --------------------------------------------------------- |
| **스킬 파일 직접 로드** | 오케스트레이터가 `cat .codex/skills/{name}/SKILL.md`로 읽어 프롬프트에 주입          | 스킬이 독립 워크플로우이고 사용자가 직접 호출 가능한 경우 |
...
**원칙:** 오케스트레이터는 필요한 스킬 파일을 shell `cat`으로 읽어 에이전트 프롬프트에 주입한다. 행동 원칙·프로토콜·체크리스트는 거의 항상 스킬로 분리해야 재사용성·유지보수성이 높아진다.
