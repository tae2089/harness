...
| 역할 티어                                  | 사용 모델 ID            | 비고                                                    |
| ------------------------------------------ | ----------------------- | ------------------------------------------------------- |
| 오케스트레이터, Architect (복잡 추론·설계) | `gpt-5.2-thinking`      | 256K ctx, 심층 추론·연구·다단계 계획                    |
| 워커 (Coder·Analyst·Reviewer·Operator)     | `gpt-5.3-codex`         | 1M ctx, 대형 코드베이스·리팩토링·80% SWE-Bench          |
| State Manager (CRUD 전담)                  | `gpt-5.3-chat-latest`   | 400K ctx, 고속·저비용 ($0.30/1M) — 단순 파일 읽쓰기만  |
