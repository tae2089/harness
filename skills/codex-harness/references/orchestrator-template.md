...
> **주의:** 서브에이전트 호출은 Codex subagent spawn 사용. 병렬: 기본값. 순차: 스킬 지시로 단계 분리. 병렬 실행, 셸 백그라운드(dev server 등). Claude Code와의 API 차이 상세(`TeamCreate`·`SendMessage`·`TaskCreate` 부재 이유): `references/agent-design-patterns.md` § "실행 모드: 오케스트레이션" 참조.
...
¹ 컨텍스트 한도 소진 후 자동 재개 경로. 수동 개입 불필요.
...
| 범위 불명확                       | `사용자 확인 요청("수정 범위 확인: 어떤 에이전트·스킬·파일?")` → RETURN |

---

### Step 1: 초기화

1. `user_input`에서 `{plan_name}` 파싱. 없으면 `사용자 확인 요청("작업 이름을 지정해주세요.")` → RETURN.
...
3. **스키마 템플릿 동기화 (필수):** 본 스킬의 `references/schemas/` 디렉터리에서 5개 파일을 읽어 `_workspace/_schemas/`에 그대로 작성한다. 각 파일에 대해 shell `cat` → `apply_patch` 1쌍씩 실행:
   ...
   > **`shell cp ...` 금지** — 런타임 워킹 디렉터리(사용자 프로젝트 루트)에서 스킬 reference 경로는 셸로 도달 불가. 반드시 shell `cat` + `apply_patch` 사용.
...
8. **workflow.md 스키마 검증 (작성 직후 필수, 사이클 검증보다 먼저 실행):**
   ...
   | 검사 항목                                                                                                                  | 방법                                                                                                                                                                                           | 실패 시 액션                                                                                                                                                   |
   | -------------------------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
   | Stage 필드: `종료 조건`, `다음 stage`, `사용자 승인 게이트`                                                                | 정규식/파서로 헤더 아래 필드 존재 확인                                                                                                                                                         | `사용자 확인 요청("workflow.md Stage {name} 필수 필드 누락: {missing_fields}. 보강 후 재시도.")` → HALT                                                                |
   | Step 필드: `패턴`, `활성 에이전트`, `종료 조건`, `다음 step`, `최대 반복 횟수`                                             | 동일                                                                                                                                                                                           | 동일                                                                                                                                                           |
   | `패턴` 값 = 7대 중 1 (`pipeline`·`fan_out_fan_in`·`expert_pool`·`producer_reviewer`·`supervisor`·`hierarchical`·`handoff`) | enum 검사                                                                                                                                                                                      | `사용자 확인 요청("패턴 값 위반: {value}. 7대 중 선택.")` → HALT                                                                                                       |
   | `활성 에이전트` 형식 = `[@name, ...]`                                                                                      | 정규식 `\[(@\w[\w-]*\s*,?\s*)+\]`                                                                                                                                                              | `사용자 확인 요청("활성 에이전트 형식 위반.")` → HALT                                                                                                                  |
   | **`종료 조건` 검증 가능 술어**                                                                                             | 키워드 화이트리스트: `task_*.json`·`status=done`·`존재`·`verdict=`·`score ≥`·`iterations ≥`. 화이트리스트 미매칭 + LLM 자의 해석 키워드(`승인`·`충분`·`완료되면`·`만족`·`적절히`) 발견 시 위반 | `사용자 확인 요청("Step {name}의 종료 조건이 자연어다('{value}'). 검증 가능 술어로 재작성: 파일 존재·JSON 필드값·iteration ≥ N.")` → HALT                              |
   | 사용자 승인 게이트 누락                                                                                                    | Stage 블록에 명시 안 됨                                                                                                                                                                        | 동일                                                                                                                                                           |
   | **Stage·Step 명명 규칙 (Jira 제목 컨벤션)**                                                                                | 헤더 이름 추출 → 정규식 `^[a-z][a-z0-9-]*$` 일치 + placeholder 블랙리스트(`main`·`step1`·`task`·`work`·`default`·`phase1`·`stage1`·`generic`) 미포함                                           | `사용자 확인 요청("Stage/Step 이름 위반: '{name}'. kebab-case + deliverable 명사구로 재작성 (예: sso-integration, requirements-gathering). placeholder 금지.")` → HALT |

9. **workflow.md 사이클 검증 (스키마 검증 통과 후):**

   | 검사 항목              | 방법                                                                                           | 실패 시 액션                                                                                     |
   | ---------------------- | ---------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------ |
   | Step 내 순환 참조      | 각 Stage 내 Step의 `다음 step` 링크를 순서대로 추적 — 이미 방문한 Step 이름이 재등장 시 → 순환 | `사용자 확인 요청("workflow.md에 순환 참조 발견: {Stage} → {cycle_path}. 수정 후 재시도하세요.")` → HALT |
...
4. **심볼릭 플레이스홀더 해결:** `active_agents`에 `@선택된_전문가` 등 심볼릭 이름 포함 시:
   ...
   - 필드 미존재 → `사용자 확인 요청("expert_pool Step이 아직 실행되지 않았거나 selected_expert 미기록. 어떤 에이전트를 호출할까요?")` → RETURN.
...
| 패턴                         | 호출 방식                                                                                 | 완료 후 기록                                                         |
| ---------------------------- | ----------------------------------------------------------------------------------------- | -------------------------------------------------------------------- |
| `pipeline` / `hierarchical`  | 순차 호출, 에이전트마다 즉시 기록                                   | findings.md, tasks.md, checkpoint.json                               |
| `fan_out` / `fan_out_fan_in` | 병렬 호출 → 전체 완료 후 `COLLECT ALL task_*.json` → apply_patch ⁷ | tasks/task*{agent}*{id}.json, tasks.md, findings.md, checkpoint.json |
...
2. AMBIGUOUS → `사용자 확인 요청("전문가 목록: {active_agents}")` → RETURN.
3. findings.md[라우팅 근거] 기록 → checkpoint `shared_variables.selected_expert` 갱신 → 에이전트 호출 → task 파일 기록.

⁶ **handoff 상세:**

1. `active_agents[0]` 호출.
2. 응답에 `[NEXT_AGENT: @{name}]` 포함 시: `handle_handoff({name})` 순환 감지 → next*prompt 구성 → `{name}` 호출 → `task*{name}\_{id}.json` 기록.
