...
> **번들 검증:** Step 1.3 가 `references/schemas/` 9종을 `read_file`로 읽으므로 누락 시 즉시 런타임 실패. 스킬 생성 직후 `ls .codex/skills/{name}/references/schemas/`로 9개 파일 존재 확인.
...
| **M6. schemas/ 번들** | `codex-harness/references/schemas/` 9종을 신규 스킬 `references/schemas/`로 복사 (task/checkpoint/workflow/findings/tasks 스키마 + models.md + agent-worker/orchestrator/state-manager 템플릿). | `ls .codex/skills/{new}/references/schemas/` 9개 파일 존재 | 번들 완료 |
