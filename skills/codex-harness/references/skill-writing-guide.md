...
7. [데이터 브로커링 & 조율 패턴 (Codex CLI 고유)](#7-데이터-브로커링--조율-패턴-gemini-cli-고유)
...
Description은 스킬의 **유일한 트리거 메커니즘**이다. Codex CLI 트리거 라우터는 `.gemini/skills/*/SKILL.md`의 `name` + `description`만 보고 스킬 사용 여부를 결정한다.
