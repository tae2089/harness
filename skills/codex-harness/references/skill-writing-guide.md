...
- `scripts/` — 에이전트들이 공통으로 작성하는 스크립트를 미리 번들링. 셸에서 직접 실행.
...
Description은 스킬의 **유일한 트리거 메커니즘**이다. Codex CLI 트리거 라우터는 `.agents/skills/*/SKILL.md`의 `name` + `description`만 보고 스킬 사용 여부를 결정한다.
...
번들링된 스크립트는 반드시 **실행 테스트**를 거친다. 셸에서 실제 실행 결과를 확인한 뒤 스킬에 포함한다.
...
### 7-4. 절차형 스킬은 오케스트레이터가 명시적으로 로드하는 것을 전제

Codex CLI에서 스킬은 오케스트레이터가 `cat .agents/skills/{name}/SKILL.md`로 읽어 프롬프트에 주입하는 방식으로 활성화된다. 스킬 작성 시 **다른 스킬을 참조·재사용**한다면 해당 스킬 이름과 로드 시점을 명시한다.
...
- [ ] 에러 핸들링 (Zero-Tolerance: 재시도 ≤2 → Blocked + 사용자 확인 요청)
...
```
{프로젝트}/.agents/skills/{name}-orchestrator/
├── SKILL.md
└── references/
    └── schemas/
...
> **번들 검증:** Step 1.3 가 `references/schemas/` 9종을 shell `cat`로 읽으므로 누락 시 즉시 런타임 실패. 스킬 생성 직후 `ls .agents/skills/{name}/references/schemas/`로 9개 파일 존재 확인.
...
| **M6. schemas/ 번들** | `codex-harness/references/schemas/` 9종을 신규 스킬 `references/schemas/`로 복사 (task/checkpoint/workflow/findings/tasks 스키마 + models.md + agent-worker.template.toml + agent-orchestrator.template.md + agent-state-manager.template.toml). | `ls .agents/skills/{new}/references/schemas/` 9개 파일 존재 | 번들 완료 |
