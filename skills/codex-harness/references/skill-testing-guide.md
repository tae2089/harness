...
**권장 도구:** 사용자 확인 요청, 스킬 로드, shell `cat`, `grep_search`, `run_shell_command` (`temperature: 0.2`).
...
> Codex CLI 자동화는 `codex -p "..."`를 `run_shell_command`로 호출하는 스크립트로 수행한다. 토큰 비용이 높으므로 스킬이 충분히 안정화된 후 최종 단계에서 실행한다.
