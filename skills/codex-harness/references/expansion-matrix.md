...
| **task_*.json 영속 미사용** (tasks.md 단순 체크만) | 병렬 에이전트 보고가 tasks.md 직접 수정으로 대체됨 — race condition 위험 | 각 에이전트에게 `_workspace/tasks/task_{agent}_{id}.json` 작성 의무 부여. 메인이 GLOB 수집 후 apply_patch로 tasks.md 갱신 |
