...
**다단계 하네스 구축 전 항상 Plan Mode 먼저 진입한다.**

- 활성화: `/plan` 또는 `Shift+Tab`
- 목적: 컨텍스트 수집 → 확인 질문 → 강한 계획 수립 후 구현
...
> `model_reasoning_effort` 기준: `low`(StateManager) / `medium`(Analyst·Researcher) / `high`(Coder·QA·Reviewer) / `extra-high`(Orchestrator·Architect). 상세: `references/schemas/models.md`.
