...
- **조율:** `tasks.md`를 활용한 상태 관리. 메인이 `Todo` 목록을 쪼개 각 워커에게 배치 할당, 워커 완료 후 `Done`으로 갱신, 다음 배치 할당.
- **사용 예시 (오케스트레이터의 호출 방식):**
  ...
  > (완료 보고가 오면)
  > **메인:** `[Tool: invoke_agent(agent_name="worker-1", wait_for_previous=true)]` `@{worker-1}`님, 완료 확인했습니다. 이제 21~30번 작업을 수행하세요.
