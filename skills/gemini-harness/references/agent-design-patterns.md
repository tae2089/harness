...
- **주의:** 순환 핸드오프(A→B→A) 무한 루프 방지 필수 — 오케스트레이터는 호출 이력을 추적하고 동일 에이전트가 이미 실행됐으면 재소환하지 않는다. 핸드오프 연쇄가 3단계 이상 발생하거나 추천 대상 에이전트가 없으면 `ask_user`로 사용자에게 결정권을 넘긴다.
...
- **필수 포함 데이터:**
  - `last_successful_phase`: 마지막으로 성공한 Phase 번호 또는 에이전트명.
  - `tasks_snapshot`: `tasks.md`의 현재 상태 데이터 요약.
  - `shared_variables`: 에이전트 간 공유된 핵심 상수나 변수 값.
- **장점:** 실행 중 중단되더라도 Phase 0에서 체크포인트를 감지하여 **실패 지점부터 즉시 재개** 가능하다.
...
| 에이전트 역할                         | 권장 모델                   |
| ------------------------------------- | --------------------------- |
| 오케스트레이터, Architect             | `gemini-3.1-pro-preview`    |
| Coder, Analyst, Reviewer, Operator    | `gemini-3-flash-preview`    |
| 모델 고정 없이 메인 세션 상속         | `inherit`                   |

> **이유:** 워커 에이전트는 범위가 좁고 반복 호출이 많다. Flash는 Pro 대비 속도·비용 우위. 복잡한 설계·판단이 필요한 오케스트레이터·Architect만 Pro를 사용한다.
...
kind: local
model: "gemini-3-flash-preview"  # 워커(Analyst·Coder·Reviewer·Operator). 오케스트레이터·Architect는 "gemini-3.1-pro-preview"
tools:
  - ask_user
  - activate_skill
  - read_file
