# 사용 사례: gemini-harness 트리거 발화 → 처리 경로 카탈로그

`gemini-harness`를 호출해 하네스를 구축·확장·운영할 때의 **실전 발화 패턴 8종**과 각 발화에 대한 처리 경로(모드 분기 → 패턴 선택 → workflow.md 구조 → 산출물). 새 도메인을 받았을 때 어느 시나리오에 가장 가까운지 매칭하여 참고한다.
...
`gemini-harness`는 트리거 직후 Phase 0에서 모드를 분기한다 (SKILL.md Phase 0 참조). 발화 패턴별 모드:

| 발화 패턴 | 모드 | 진입 Phase | 대표 키워드 |
|-----------|------|-----------|------------|
| "X 도메인 하네스 구성/구축/설계해줘" | **신규 구축** | Phase 1 → Phase 6 전체 | "구성", "만들어줘", "설계", "구축", "셋업" |
...
다음 발화는 **gemini-harness를 호출하지 않는다.** 직접 응답 또는 다른 스킬 호출.
