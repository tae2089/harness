...
## 2. 오케스트레이터 SKILL.md (`.gemini/skills/sso-dev-flow/SKILL.md`)
...
5. iterations=3 도달 시 `Blocked` + ask_user.

## 에러 핸들링
Zero-Tolerance: 에이전트 실패 → 최대 2회 재시도 → 미해결 시 `task_*.json` status=Blocked + ask_user. 임의 Skip 금지.
```
...
| task_*.json 영속 | 미사용 | `_workspace/tasks/task_*.json` 표준 |
