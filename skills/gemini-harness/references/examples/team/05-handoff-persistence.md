...
Phase 1: workflow.md 생성:
         ...
         checkpoint.json 생성 (current_stage: "main", current_step: "triage",
                            active_pattern: "handoff", status: "in_progress")
         tasks.md 초기화 (fix 작업 등록용)
         findings.md [공유 변수/경로]·[데이터 충돌] 초기화
Phase 2: [Step 실행 루프 — Stage main / Step triage]
...
Phase 3: [Step 실행 루프 — Stage main / Step fix]
         ...
         step "fix" 종료 조건 충족 → stage "main" 완료
         checkpoint.json 갱신:
