...
> **주의:** 서브에이전트 호출은 `invoke_agent` 사용. 병렬 실행: `wait_for_previous: false`. 셸 백그라운드(dev server 등): `run_shell_command`. Claude Code와의 API 차이 상세(`TeamCreate`·`SendMessage`·`TaskCreate` 부재 이유): `references/agent-design-patterns.md` § "실행 모드: 오케스트레이션" 참조.
