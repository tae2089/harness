...
GEMINI.md 변경 이력: "@security-reviewer 추가 | 보안 검토 부재 피드백"
```
...
GEMINI.md 변경 이력: "code-review 스킬 체크리스트 강화 | 리뷰 품질 피드백"
```
...
GEMINI.md 변경 이력: "아키텍처 Pipeline→Fan-out/Fan-in 변경 | 분석 속도 개선 목적"
```
...
|------------|-----------|-----------|
| **에이전트 파일 존재 / GEMINI.md 미등록** | 에이전트 추가 후 GEMINI.md 변경 이력 미기록 | GEMINI.md 변경 이력 보정 후 Phase 6-1 재검증 |
| **GEMINI.md 등록 / 에이전트 파일 없음** | 파일 삭제 누락 또는 이름 변경 미반영 | `ask_user`로 의도 확인 후 파일 복구 또는 GEMINI.md 항목 삭제 |
...
| **오케스트레이터 SKILL.md 평면 Step 나열 (Stage 계층 부재)** | 초기 생성 시 Stage-Step 모델 미적용 (예: `examples/sso-dev-flow`처럼 `Step 0~4` 평면 헤더) | Phase 5 부분 실행 — Stage 블록으로 재구조화. 종료 조건 자연어 → 검증 가능 술어 변환. workflow.md 신규 작성. `references/examples/full-bundle/sso-style.md` 정본 패키지 참조. |
...
**drift 해소 원칙:** drift 항목이 1개라도 있으면 변경 유형 Phase 실행 전에 반드시 해소한다. drift 해소 자체도 Phase 6-1(구조 검증)로 확인 후 GEMINI.md 변경 이력에 기록한다.
