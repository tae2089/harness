...
7대 아키텍처 패턴의 실제 구현 예시. **Gemini CLI 환경에서는 서브에이전트 간 직접 통신이 불가능**하므로, 모든 팀 통신은 메인 에이전트가 `_workspace/findings.md`·`_workspace/tasks.md`를 통해 중개한다.
...
- 입력: 사용자의 세계관 컨셉, `_workspace/{plan_name}/00_input/brief.md`의 장르 요구사항
- 출력: `_workspace/{plan_name}/01_worldbuilder_setting.md`
...
- science-consultant의 피드백은 `_workspace/{plan_name}/03_science_review.md`에 기록되며,
...
```
Phase 1: @webtoon-artist 호출 → _workspace/{plan_name}/panels/*.png 생성
Phase 2: @webtoon-reviewer 호출 → _workspace/{plan_name}/review_report.md 생성
...
Phase 4: 최종 PASS/강제 PASS 집계 → _workspace/{plan_name}/final/episode.md 생성
```
...
| 에이전트              | 유형     | 핵심 도구                                                                     | 산출물                             |
| --------------------- | -------- | ----------------------------------------------------------------------------- | ---------------------------------- |
| @security-reviewer    | Reviewer | `ask_user`, `activate_skill`, `read_file`, `grep_search`, `run_shell_command` | `_workspace/{plan_name}/review_security.md`    |
| @performance-reviewer | Reviewer | `ask_user`, `activate_skill`, `read_file`, `grep_search`, `run_shell_command` | `_workspace/{plan_name}/review_performance.md` |
| @test-reviewer        | Reviewer | `ask_user`, `activate_skill`, `read_file`, `grep_search`, `run_shell_command` | `_workspace/{plan_name}/review_tests.md`       |
...
1차 라운드 (병렬):
  ...
  invoke_agent(wait_for_previous: false)로 배치 호출
  → 각자 _workspace/{plan_name}/review_*.md 생성
...
2차 라운드 (병렬):
  ...
  → 각자 _workspace/{plan_name}/review_*_v2.md 생성

통합:
  메인 에이전트가 최종 리포트 _workspace/{plan_name}/final/code_review.md 생성
```
...
Phase 3: 각 산출물을 _workspace/{plan_name}/market_{country}.md에 저장
Phase 4: 메인 에이전트가 통합 보고서 _workspace/{plan_name}/final/market_report.md 생성
```
...
```
Phase 1: @project-architect 호출 → 전체 설계 _workspace/{plan_name}/00_architecture.md
...
## 예시 8: 시스템 디버깅 및 장애 대응 팀 (Handoff + Persistence 패턴)
...
```
Phase 0: _workspace/checkpoint.json 확인
         - 존재 시: "마지막 분석 로그 라인: 50,000" 확인 후 그 지점부터 분석 재개 (Persistence)
         - 미존재 시: 신규 분석 시작

Phase 1: @incident-triage 호출 → 수 기가바이트의 로그 및 스택 트레이스 스캔
         - 분석 결과: "DB 연결 타임아웃 발견"
         - 응답 끝에 핸드오프 신호 삽입: 
           `[NEXT_AGENT: @db-fixer] 사유: Connection Pool 고갈 및 Slow Query 확인됨.`

Phase 2: 오케스트레이터가 [NEXT_AGENT] 파싱
         - 계획된 일반 로직 가이드 대신 즉시 @db-fixer 호출
         - 호출 프롬프트에 트리아지의 분석 요약과 로그 위치 자동 주입

Phase 3: @db-fixer가 수정 수행 후 @incident-triage에게 재검증 요청 (Fix Loop)

Phase 4: 최종 조치 완료 보고 및 checkpoint.json을 'Resolved'로 마킹
```
...
- SQL/DB 에러 발견 시: `[NEXT_AGENT: @db-fixer]`
- 403/401/인증 에러 발견 시: `[NEXT_AGENT: @security-patcher]`
- 그 외 로직 오류: `[NEXT_AGENT: @logic-fixer]`

## 영속성 가이드
로그 분석 중 `max_turns`에 도달할 경우, 현재까지 읽은 마지막 파일 경로와 라인 번호를 
`_workspace/findings.md`의 [공유 변수] 섹션에 기록하여 다음 턴에서 이어서 읽을 수 있게 하라.
```
