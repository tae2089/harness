...
> **주의:** Claude Code의 `TeamCreate`·`SendMessage`·`TaskCreate` 같은 팀 API는 Gemini CLI에 존재하지 않는다. 서브에이전트 호출은 **`invoke_agent` 도구**를 사용하며, 병렬 실행이 필요한 경우 **`wait_for_previous: false`** 파라미터를 지정하여 구현한다.
