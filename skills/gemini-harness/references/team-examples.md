...
> Claude Code의 `TeamCreate`·`SendMessage`·`TaskCreate`·`TaskUpdate` 같은 팀 API는 Gemini CLI에 없다. 본 사례들은 해당 기능을 **파일 기반 브로커링**으로 치환해 재구성한 것이다.
...
Phase 2: 단일 응답 턴에서 @official/@media/@community/@background 배치 호출
...
Phase 3: @science-consultant와 @continuity-manager 배치 호출. 입력은 02_prose_draft.md
...
```
Phase 1: @webtoon-artist → _workspace/panels/*.png 생성
Phase 2: @webtoon-reviewer → _workspace/review_report.md 생성
...
```
1차 라운드 (병렬):
  @security-reviewer / @performance-reviewer / @test-reviewer 배치 호출
  → 각자 review_*.md 생성
...
2차 라운드 (병렬):
  3명의 리뷰어를 재호출하면서 findings.md [데이터 충돌]의 해당 항목을 프롬프트에 주입
...
Phase 2: 메인 에이전트가 tasks.md를 읽어 가용 워커 수만큼 배치 할당
         - 단일 응답 턴에서 @migrator-1 / @migrator-2 / @migrator-3 배치 호출
...
Phase 2: 식별된 국가에 해당하는 @market-analyst-* 만 선택 호출
...
```
Phase 1: @project-architect → 전체 설계 _workspace/00_architecture.md
         → findings.md [공유 변수/경로]에 프론트/백엔드 계약 요약

Phase 2: @frontend-team-lead / @backend-team-lead 배치 호출
         → 각자 자기 영역 설계 + 필요한 하위 작업을 tasks.md에 추가

Phase 3: 메인 에이전트가 tasks.md를 읽어 @ui-designer·@state-engineer·
         @api-designer·@db-engineer를 순차/병렬 호출
         → 산출물은 각 팀 리드가 통합 검수
