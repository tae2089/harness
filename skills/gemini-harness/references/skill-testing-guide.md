...
Gemini CLI는 **서브에이전트 병렬 실행 플래그**가 없으므로, 에이전트 차원의 병렬성은 **한 응답 안에서 에이전트를 연속 호출**하는 배치 패턴으로만 확보한다. (셸 명령은 `run_shell_command`의 백그라운드 옵션으로 별개로 동시 실행 가능하지만, 평가용 에이전트 호출과는 구분된다.)
...
이 스키마는 `_workspace/iteration-N/eval-{id}/{variant}/grading.json` 경로에 저장하며, 하네스 `SKILL.md` Phase 5에서 참조한다.
