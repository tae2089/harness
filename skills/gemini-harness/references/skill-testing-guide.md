...
Gemini CLI에서는 서브에이전트 간 통신이 없으므로, **메인 에이전트가 Grader·Analyzer 역할을 겸하거나 전용 QA 에이전트(`@grader`)를 호출**해 채점을 수행한다.
...
Gemini CLI에서 에이전트 차원의 병렬성은 **`invoke_agent` 도구 호출 시 `wait_for_previous: false`** 파라미터를 지정하여 확보한다. (셸 명령은 `run_shell_command`의 백그라운드 옵션으로 별개로 동시 실행 가능하지만, 평가용 에이전트 호출과는 구분된다.)
...
Gemini CLI는 실행 결과에 토큰/시간 메타데이터가 포함되므로, 메인 에이전트는 이를 `_workspace/{skill-name}/iteration-N/eval-{id}/{variant}/timing.json`에 `write_file`로 기록한다.
...
Gemini CLI에서는 메인 에이전트가 `run_shell_command`로 검증 스크립트(파이썬·쉘 등)를 실행해 결과를 `grading.json`에 기록한다.
...
이 스키마는 `_workspace/{skill-name}/iteration-N/eval-{id}/{variant}/grading.json` 경로에 저장하며, 하네스 `SKILL.md` Phase 5에서 참조한다.

---

## 5. 전문 에이전트 활용 (Grader / Comparator / Analyzer)

Gemini CLI 환경에서는 서브에이전트 간 직접 통신이 불가능하므로, 평가 전용 에이전트들을 **메인 에이전트가 순차 호출**하여 결과를 `findings.md`·`grading.json`으로 중개한다.
...
**Gemini CLI 주의:** 메인 에이전트는 두 산출물을 `_workspace/.../variant_A.md`·`variant_B.md`로 익명화 복사한 뒤 Comparator에 경로만 전달해야 한다. 프롬프트에 "with_skill"/"without_skill" 라벨이 노출되면 블라인드성이 깨진다.
...
Gemini CLI의 트리거 라우터는 description만 보고 스킬을 선택한다. 따라서 description이 스킬의 **유일한 실질적 API**다.
...
> Gemini CLI 자동화는 `gemini -p "..."`를 `run_shell_command`로 호출하는 스크립트로 수행한다. 토큰 비용이 높으므로 스킬이 충분히 안정화된 후 최종 단계에서 실행한다.
