...
1. **1~2회 재시도(총 3회):** 실패 원인을 파악하고 접근법을 바꿔 재시도. 매 재시도마다 `retries` 값 증가.
2. **3회 후에도 실패 → `Blocked`:** `status: "Blocked"`, `result: null`로 기록. `qa_report.md`의 "차단 항목" 섹션에 실패 원인·시도 이력·필요한 정보를 상세 기록.
