...
`_workspace/tasks.md`를 업데이트하는 절차를 스킬에 포함하여 전체 워크플로우의 가시성을 확보한다. 스킬이 실행을 마치면 반드시 해당 태스크 ID의 상태(`Todo → In-Progress → Done`)와 산출물 경로를 기록한다.
...
- `tasks.md`: `[ID]`, `[에이전트]`, `[작업 내용]`, `[상태]`, `[연결 산출물]`
