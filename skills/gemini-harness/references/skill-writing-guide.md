...
신규 오케스트레이터 스킬은 런타임에 자기 `references/schemas/`를 읽으므로 (`gemini-harness`는 메타 스킬·런타임 활성화 안 됨) 다음 **8종**을 반드시 번들링한다. SoT는 `gemini-harness/references/schemas/` — 신규 스킬 생성 시 그대로 복사.
...
> **번들 검증:** Step 1.3 가 `references/schemas/` 8종을 `read_file`로 읽으므로 누락 시 즉시 런타임 실패. 스킬 생성 직후 `ls .gemini/skills/{name}/references/schemas/`로 8개 파일 존재 확인.
...
| **M6. schemas/ 번들** | `gemini-harness/references/schemas/` 8종을 신규 스킬 `references/schemas/`로 복사 (task/checkpoint/workflow/findings/tasks 스키마 + models.md + agent-worker/orchestrator 템플릿). | `ls .gemini/skills/{new}/references/schemas/` 8개 파일 존재 | 번들 완료 |
