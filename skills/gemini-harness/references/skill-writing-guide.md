...
- `references/` — 자주 트리거되지 않는 상세 내용을 분리. 에이전트가 `read_file`로 조건부 로드.
...
신규 오케스트레이터 스킬은 런타임에 자기 `references/schemas/`를 읽으므로 (`gemini-harness`는 메타 스킬·런타임 활성화 안 됨) 다음 **9종**을 반드시 번들링한다. SoT는 `gemini-harness/references/schemas/` — 신규 스킬 생성 시 그대로 복사.

```
{프로젝트}/.gemini/skills/{name}-orchestrator/
├── SKILL.md
└── references/
    └── schemas/
        ├── task.schema.json                    ← gemini-harness/references/schemas/ 사본
        ...
        └── agent-state-manager.template.md     ← 상태 관리 에이전트 생성 기준 (선택적 사용)
```

> **번들 검증:** Step 1.3 가 `references/schemas/` 9종을 `read_file`로 읽으므로 누락 시 즉시 런타임 실패. 스킬 생성 직후 `ls .gemini/skills/{name}/references/schemas/`로 9개 파일 존재 확인.
> **갱신 정책:** gemini-harness `references/schemas/` 변경 시 모든 파생 오케스트레이터 스킬에 동일 변경 전파(드리프트 방지). 진행 중인 워크스페이스의 `_workspace/_schemas/`는 스냅샷이므로 보존.
...
| **M6. schemas/ 번들** | `gemini-harness/references/schemas/` 9종을 신규 스킬 `references/schemas/`로 복사 (task/checkpoint/workflow/findings/tasks 스키마 + models.md + agent-worker/orchestrator/state-manager 템플릿). | `ls .gemini/skills/{new}/references/schemas/` 9개 파일 존재 | 번들 완료 |
...
- GEMINI.md 변경 이력에 마이그레이션 기록 필수: `[YYYY-MM-DD] {스킬명}: 평면 Step → Stage-Step 전환 (M1~M6)`.
